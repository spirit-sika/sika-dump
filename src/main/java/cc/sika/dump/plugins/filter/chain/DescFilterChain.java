...
/**
 * 倒序过滤器链, 由多个OrderableFilter构建而成
 * <p>
 * 在升序过滤器链中, 过滤器的order越小越晚执行
 *
 ...
 */
@Getter
public class DescFilterChain<T> {

    private final Collection<OrderableFilter<T>> filters;
    private final List<OrderableFilter<T>> chian;
    private int position;

    public DescFilterChain(Collection<OrderableFilter<T>> filters) {
        if (filters == null || filters.isEmpty()) {
            throw new IllegalArgumentException("filters can not be empty");
        }
        this.filters = filters;
        this.chian = filters.stream().sorted((o1, o2) -> Integer.compare(o2.getOrder(), o1.getOrder())).toList();
        this.position = 0;
    }
